LOVELY_INTEGRITY = 'e6d78211cbc1fd8d92ab6470623b0fe413d80727bc1ab1a83e0029a9dd4596b9'
...
G.FUNCS.your_collection_voucher_page = function(args)
  ...
  for i = 1, 4 do
    for j = 1, #G.your_collection do
      ...
      if not center then break end
      local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w/2, G.your_collection[j].T.y, G.CARD_W, G.CARD_H, G.P_CARDS.empty, center)if FlowerPot.CONFIG.voucher_sticker_enabled < 3 then card.sticker = get_voucher_win_sticker(center) end
      card:start_materialize(nil, i>1 or j>1)
      G.your_collection[j]:emplace(card)
    end
  end
